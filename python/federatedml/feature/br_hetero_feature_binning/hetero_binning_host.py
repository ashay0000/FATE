...
from federatedml.feature.br_hetero_feature_binning.base_feature_binning import BaseHeteroFeatureBinning # 经过修改
...
class HeteroFeatureBinningHost(BaseHeteroFeatureBinning):
    def fit(self, data_instances):
        ...
        # 新增内容 #
        woe = self.transfer_variable.woe.get(idx=-1)
        idx_dict = {v: k for k, v in self.bin_inner_param.col_name_maps.items()}
        bin_result = {}
        for i in woe[0]:
            idx = int(i.split('_')[-1])
            bin_result[idx_dict[idx]] = woe[0][i]

        self.binning_obj.bin_results.all_cols_results = bin_result
        # 新增内容 #
